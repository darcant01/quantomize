// api/sales.js — POS, sales, reports
  ...
  try {
    ...
    if (action === 'createSale') {
      ...
      for (const item of items) {
        const { data: product } = await supabase
          .from('products').select('stock, name').eq('id', item.productId).single();
      ...
      for (const item of items) {
        const { data: product } = await supabase
          .from('products').select('name, cost_price, stock').eq('id', item.productId).single();

        await supabase.from('sale_items').insert({
          sale_id: sale.id, product_id: item.productId,
          product_name: product.name, qty: item.qty,
          price: item.price, cost: product.cost_price,
    ...
    if (action === 'voidSale') {
      ...
      if (sale.status === 'voided')
        return res.status(400).json({ success: false, error: 'Already voided' });
    ...
    if (action === 'getReports') {
      ...
      if (type === 'summary') {
        ...
        for (const sale of sales || []) {
          ...
          for (const item of sale.sale_items || []) {
            cogs += Number(item.cost) * Number(item.qty);
          }
        ...
        return res.json({
          success: true,
          summary: {
            totalRevenue, grossProfit: totalRevenue - cogs,
            totalDiscount, totalOrders, cogs,
            avgOrderValue: totalOrders ? totalRevenue / totalOrders : 0,
            daily, lowStock: lowStock || []
          }
        });
      }
      ...
      if (type === 'products') {
        const { data: items } = await supabase
          .from('sale_items')
          .select('product_id, product_name, qty, price, cost, line_total, sales!inner(status, created_at)')
          .neq('sales.status', 'voided')
          .gte('sales.created_at', fromD)
          .lte('sales.created_at', toD);

        const prodMap = {};
        for (const item of items || []) {
          if (!prodMap[item.product_id]) {
            prodMap[item.product_id] = { name: item.product_name, qty: 0, revenue: 0, profit: 0 };
          }
      ...
      if (type === 'staff') {
        ...
        const staffMap = {};
        for (const sale of sales || []) {
          if (!staffMap[sale.user_id]) {
            staffMap[sale.user_id] = { userId: sale.user_id, username: sale.username, sales: 0, revenue: 0 };
          }
        ...
        return res.json({
          success: true,
          staff: Object.values(staffMap).sort((a, b) => b.revenue - a.revenue)
        });
      }
    }
