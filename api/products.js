// api/products.js — Products, Categories, Stock
  ...
  try {
    ...
    if (action === 'addProduct') {
      ...
      const { data, error } = await supabase.from('products').insert({
        ...
        unit: unit || 'pcs',
        low_stock_alert: Number(low_stock_alert) || 5,
      ...
      if (Number(stock) > 0) {
        await supabase.from('inventory_log').insert({
          product_id: data.id, product_name: name,
          type: 'INITIAL', qty: Number(stock), prev_stock: 0,
          reason: 'Initial stock', created_by: profile.username
        });
      }
      return res.json({ success: true, id: data.id });
    }
    ...
    if (action === 'getInventoryLog') {
      ...
      let query = supabase.from('inventory_log').select('*').order('created_at', { ascending: false }).limit(500);
